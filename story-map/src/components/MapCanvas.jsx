...
import { useAnimatedConnectionLines } from './AnimatedConnectionLines.jsx'
...
export default function MapCanvas({ onReady, grayscale = false, showVideoPoints = false, scrollContainer = null, visibleSiteIds = [], currentScene = null, isMobile = false }) {
  const containerRef = useRef(null)
  const mapRef = useRef(null)
  const markersRef = useRef([])
  const visibleSiteIdsRef = useRef(visibleSiteIds) // Track current value for async access
  const isMobileRef = useRef(isMobile) // Track current isMobile value for event listeners
  const [markersReady, setMarkersReady] = useState(false) // Track when markers are loaded
  const [mapLoaded, setMapLoaded] = useState(false) // Track when map is fully loaded
  const [tooltip, setTooltip] = useState({ visible: false, x: 0, y: 0, text: '' })
  
  // Update isMobile ref when it changes
  useEffect(() => {
    isMobileRef.current = isMobile
  }, [isMobile])
  
  // Use animated connection lines for globe scenes
  useAnimatedConnectionLines(
    ...
    map.on('load', async () => {
      ...
      try {
        ...
        map.addSource('west-bank-overlay', {
          type: 'image',
          url: `${import.meta.env.BASE_URL}assets/west-bank.png`,
      ...
      // Add Naama Solar Field overlay - hidden by default, shown only on specific scenes
      try {
        const naamaSolarCoords = [
          [35.462654938710045, 31.918423364568028],  // top-left
          [35.4584266787337, 31.918423364568028],    // top-right
          [35.4584266787337, 31.91352102810771],     // bottom-right
          [35.462654938710045, 31.91352102810771]    // bottom-left
        ]

        map.addSource('naama-solar-overlay', {
          type: 'image',
          url: `${import.meta.env.BASE_URL}assets/naama-solar.png`,
          coordinates: naamaSolarCoords
        })

        map.addLayer({
          id: 'naama-solar-overlay-layer',
          type: 'raster',
          source: 'naama-solar-overlay',
          layout: {
            visibility: 'visible'  // Visible by default across all scenes
          },
          paint: {
            'raster-opacity': 1
          }
        })
        console.log('MapCanvas: Naama Solar overlay added (visible)')
      } catch (e) {
        console.error('MapCanvas: Naama Solar overlay setup error:', e)
      }

      try {
        // Use smaller pins on landing page (grayscale mode) or sub-intro scene
        const pinSize = (grayscale || currentScene === 'sub-intro' || currentScene === 'sub-intro-2' || currentScene === 'west-bank') ? 16 : 38
        console.log('MapCanvas: loading site points with pinSize:', pinSize, 'grayscale:', grayscale, 'currentScene:', currentScene)
        ...
        markers.forEach(({ marker, siteId, name, meta }) => {
          ...
          if (isMobileRef.current) {
            // On mobile: show tooltip on click and keep it visible until clicking elsewhere
        ...
        setMapLoaded(true) // Mark map as fully loaded for animated lines
        ...
        onReady?.({
          ...
          showNaamaSolarOverlay: (visible) => {
            if (map.getLayer('naama-solar-overlay-layer')) {
              map.setLayoutProperty('naama-solar-overlay-layer', 'visibility', visible ? 'visible' : 'none')
              console.log('MapCanvas: Naama Solar overlay visibility:', visible ? 'visible' : 'none')
            }
          }
        })
  ...
  // On mobile: hide tooltip when clicking elsewhere
  useEffect(() => {
    if (!isMobile || !tooltip.visible) return

    const handleDocumentClick = (e) => {
      // Check if click is on a marker element
      const clickedMarker = e.target.closest('.maplibregl-marker')
      if (!clickedMarker) {
        // Clicked outside any marker, hide tooltip
        setTooltip(t => ({ ...t, visible: false }))
      }
    }
...
*/
  ...
  useEffect(() => {
    ...
    // Update marker pin sizes based on grayscale mode or sub-intro scene
    const pinSize = (grayscale || currentScene === 'sub-intro' || currentScene === 'sub-intro-2' || currentScene === 'west-bank') ? 16 : 38
